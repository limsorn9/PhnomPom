...
export const Header: React.FC<HeaderProps> = ({
  ...
  const tabTitles: Record<ActiveTab, { title: string; subtitle: string; icon: React.ComponentType<{ className?: string }> }> = {
    ...
    activity_history: {
      title: language === 'en' ? 'Activity History & Timeline' : 'ប្រវត្តិសកម្មភាព & Timeline',
      subtitle: language === 'en' ? 'Chronological audit trail of user actions, logins, edits, and exports' : 'កំណត់ត្រាតាមលំដាប់លំដោយពេលវេលានៃសកម្មភាពចូលប្រព័ន្ធ កែប្រែទិន្នន័យ និងនាំចេញ',
      icon: History
    },
    homeroom_dashboard: {
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-2 z-50 animate-in fade-in zoom-in duration-100">
                ...
                <div className="pt-1 border-t border-slate-100 dark:border-slate-800 space-y-0.5">
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 flex items-center gap-2 font-medium"
                    title="សាកល្បងបើកផ្ទាំងព្រមានផុតកំណត់ Session (២ នាទីមុនផុតកំណត់)"
                  >
                    <Clock className="w-3.5 h-3.5 text-amber-500" />
                    <span>{language === 'en' ? 'Test Session Warning (2 min)' : 'សាកល្បងផ្ទាំងព្រមាន Session'}</span>
                  </button>
