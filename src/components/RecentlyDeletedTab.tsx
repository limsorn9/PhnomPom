...
export const RecentlyDeletedTab: React.FC<RecentlyDeletedTabProps> = ({
  ...
  const filteredList = deletedUsers.filter(item => {
    const matchesSearch =
      item.user.nameKhmer.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      (item.user.staffCode && item.user.staffCode.toLowerCase().includes(searchQuery.toLowerCase())) ||
                    ...
                    <tr key={record.id} className="hover:bg-rose-50/30 transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          <img
                            src={record.user.avatarUrl || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80'}
                            alt={record.user.nameKhmer}
                            className="w-10 h-10 rounded-xl object-cover border border-slate-200 grayscale-30"
                          />
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-slate-800 text-sm">{record.user.nameKhmer}</span>
                              {getRoleBadge(record.user.role)}
                            </div>
                            <p className="text-slate-500 text-[11px] mt-0.5">{record.user.email || record.user.phone || record.user.username}</p>
                            {record.teacherProfileBackup && (
                              <span className="inline-block mt-1 text-[10px] font-semibold text-blue-700 bg-blue-50 px-2 py-0.2 rounded border border-blue-200">
                                📎 មានទិន្នន័យប្រវត្តិរូបគ្រូ (Code: {record.teacherProfileBackup.staffCode})
                              </span>
                            )}
                          </div>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-rose-200 p-6 space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              តើលោកអ្នកពិតជាចង់លុបគណនីរបស់ «<b>{selectedRecordForPurge.user.nameKhmer}</b>» ចេញពីប្រព័ន្ធជាស្ថាពរមែនទេ? ទិន្នន័យទាំងអស់នឹងត្រូវសម្អាតទាំងស្រុង។
            </p>
