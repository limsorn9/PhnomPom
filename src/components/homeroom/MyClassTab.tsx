...
export const MyClassTab: React.FC<MyClassTabProps> = ({
  ...
  const { accessStudentAccount } = useSchool();
                    ...
                    {filteredStudents.length === 0 ? (
                      ...
                      filteredStudents.map((s, idx) => (
                        ...
                        >
                          ...
                          <td className="py-2.5 px-3">
                            <p className="text-slate-700 font-medium">{s.guardianName || '—'}</p>
                            {s.guardianPhone ? (
                          ...
                          <td className="py-2.5 px-3 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  accessStudentAccount(s);
                                }}
                                className="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-[11px] font-semibold flex items-center gap-1 transition-colors cursor-pointer shadow-xs"
                                title="ចូលប្រើប្រាស់គណនីសិស្សនេះ"
                              >
                              ...
                              <button
                                ...
                                className="px-2 py-1 bg-slate-100 hover:bg-blue-100 text-slate-700 hover:text-blue-800 rounded text-[11px] font-semibold transition-colors"
                              >
                                ព័ត៌មានលម្អិត
                              </button>
                            </div>
                ...
                {filteredStudents.length === 0 ? (
                  ...
                  filteredStudents.map((s, idx) => (
                    <div
                      key={s.id}
                      onClick={() => onSelectStudent(s)}
                      className="p-3.5 space-y-2.5 bg-white hover:bg-slate-50/60 transition-colors cursor-pointer"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2.5">
                          <span className="w-6 h-6 rounded-full bg-slate-100 text-slate-600 font-times font-bold text-xs flex items-center justify-center shrink-0">
                            {idx + 1}
                          </span>
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {s.nameLatin && (
                                <span className="text-[10px] text-slate-400 font-times uppercase">
                                  {s.nameLatin}
                                </span>
                              )}
                            </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                        <div>
                          <span className="text-slate-400 text-[11px]">ថ្ងៃកំណើត៖</span>
                          <p className="font-times font-medium text-slate-700">{s.dob || '—'}</p>
                        </div>
                        <div>
                          <span className="text-slate-400 text-[11px]">អាណាព្យាបាល៖</span>
                          <p className="font-medium text-slate-700 truncate">{s.guardianName || '—'}</p>
                        </div>
                        {s.guardianPhone && (
                          <div className="col-span-2 flex items-center gap-1.5 text-blue-700 font-times font-semibold pt-0.5">
                      ...
                      <div className="flex items-center gap-2 pt-1">
                        <button
                          ...
                          className="flex-1 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-xs"
                        >
                ...
                >
                  ...
                  <div className="text-xs text-slate-600 space-y-1 bg-slate-50 p-2 rounded-lg border border-slate-100">
                    ...
                    <p className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-slate-700">{s.guardianName || '—'}</span>
                    </p>
                    <p className="flex items-center justify-between">
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      className="w-full py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-xs"
                    >
                      <UserCheck className="w-3.5 h-3.5" />
                      <span>ចូលប្រើប្រាស់គណនីសិស្ស</span>
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50/40 rounded-xl p-4 border border-amber-200/80 shadow-xs space-y-2">
              ...
              <h4 className="text-base font-bold text-slate-800 mt-2">
                {getOfficerByRole('president')?.studentNameKhmer || (classStudents[0]?.nameKhmer ?? '—')}
              </h4>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50/40 rounded-xl p-4 border border-blue-200/80 shadow-xs space-y-2">
              ...
              <h4 className="text-base font-bold text-slate-800 mt-2">
                {getOfficerByRole('vice_president')?.studentNameKhmer || (classStudents[1]?.nameKhmer ?? '—')}
              </h4>
              <p className="text-xs text-slate-500">
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50/40 rounded-xl p-4 border border-emerald-200/80 shadow-xs space-y-2">
              ...
              <h4 className="text-base font-bold text-slate-800 mt-2">
                {getOfficerByRole('discipline')?.studentNameKhmer || (classStudents[2]?.nameKhmer ?? '—')}
              </h4>
              <p className="text-xs text-slate-500">
            ...
            <div className="bg-gradient-to-br from-teal-50 to-cyan-50/40 rounded-xl p-4 border border-teal-200/80 shadow-xs space-y-2">
              ...
              <h4 className="text-base font-bold text-slate-800 mt-2">
                {getOfficerByRole('sanitation')?.studentNameKhmer || (classStudents[3]?.nameKhmer ?? '—')}
              </h4>
              <p className="text-xs text-slate-500">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50/40 rounded-xl p-4 border border-purple-200/80 shadow-xs space-y-2">
              ...
              <h4 className="text-base font-bold text-slate-800 mt-2">
                {getOfficerByRole('study')?.studentNameKhmer || (classStudents[4]?.nameKhmer ?? '—')}
              </h4>
              <p className="text-xs text-slate-500">
      ...
      {viewMode === 'seating' && (
        <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { name: 'ក្រុមទី១ «ផ្កាឈូក»', leader: classStudents[0]?.nameKhmer || '—', count: Math.ceil(classStudents.length / 4) || 0, cleanDay: 'ថ្ងៃចន្ទ', color: 'border-blue-300 bg-blue-50/40' },
              { name: 'ក្រុមទី២ «រំដួល»', leader: classStudents[1]?.nameKhmer || '—', count: Math.ceil(classStudents.length / 4) || 0, cleanDay: 'ថ្ងៃអង្គារ', color: 'border-emerald-300 bg-emerald-50/40' },
              { name: 'ក្រុមទី៣ «ចំប៉ី»', leader: classStudents[2]?.nameKhmer || '—', count: Math.ceil(classStudents.length / 4) || 0, cleanDay: 'ថ្ងៃពុធ', color: 'border-amber-300 bg-amber-50/40' },
              { name: 'ក្រុមទី៤ «កុលាប»', leader: classStudents[3]?.nameKhmer || '—', count: Math.ceil(classStudents.length / 4) || 0, cleanDay: 'ថ្ងៃព្រហស្បតិ៍', color: 'border-purple-300 bg-purple-50/40' }
            ].map((group, gIdx) => (
