...
export const RecentActivityDashboard: React.FC<RecentActivityDashboardProps> = ({
  ...
  function getDomainLabel(domain: ActivityDomain): string {
    switch (domain) {
      ...
      case 'auth':
        return 'ការផ្ទៀងផ្ទាត់ & គណនី';
      case 'system':
        return 'ប្រព័ន្ធ & ការថែទាំ';
      default:
        return domain;
    }
  }
  ...
  function getActionLabel(action: ActivityActionType): string {
    switch (action) {
      ...
      case 'login':
        return 'ចូលប្រព័ន្ធ';
      case 'logout':
        return 'ចាកចេញពីប្រព័ន្ធ';
      case 'export':
        return 'ទាញយកទិន្នន័យ (Export)';
      case 'archive':
        return 'បម្រុងទុកបណ្ណសារ (Archive)';
      case 'purge':
        return 'សម្អាតទិន្នន័យចាស់ (Purge)';
      case 'sync':
        return 'ធ្វើសមកាលកម្ម & បង្កើនល្បឿន (Sync/Vacuum)';
      default:
        return action;
    }
  }
  ...
  function getDomainIcon(domain: ActivityDomain, actionType?: ActivityActionType) {
    if (actionType === 'login') {
      return <LogIn className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />;
    }
    if (actionType === 'logout') {
      return <LogOut className="w-4 h-4 text-slate-500 dark:text-slate-400" />;
    }
    if (actionType === 'archive') {
      return <Archive className="w-4 h-4 text-amber-600 dark:text-amber-400" />;
    }
    if (actionType === 'purge') {
  ...
  function getDomainBadgeStyles(domain: ActivityDomain) {
    switch (domain) {
      ...
      case 'academic':
        return 'bg-amber-50 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800';
      case 'auth':
...
export const ActivityHistory = RecentActivityDashboard;
