...
export const GoogleDriveSyncModal: React.FC<GoogleDriveSyncModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'meetings' | 'finances' | 'history' | 'settings'>('overview');
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 no-print animate-fade-in">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-4xl w-full overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100 mt-0.5">
                សមកាលកម្មស្វ័យប្រវត្តិកំណត់ហេតុកិច្ចប្រជុំគ្រូ និងរបាយការណ៍ហិរញ្ញវត្ថុទៅកាន់ Google Drive
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex items-center gap-2 px-6 pt-3 border-b border-slate-200 bg-slate-50/80 shrink-0 overflow-x-auto text-xs">
          <button
          ...
          >
            ...
            <span>របាយការណ៍ហិរញ្ញវត្ថុ ១២ ខែ</span>
          </button>
          ...
          >
            ...
            <span>ប្រវត្តិ Sync ({driveSyncHistory.length})</span>
          </button>
              ...
              <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-950 text-white p-5 rounded-2xl shadow-lg border border-slate-800 space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    ...
                    <h4 className="font-bold text-base text-amber-300">
                      ធ្វើសមកាលកម្មឯកសារសំខាន់ៗទាំងអស់ (Full Auto-Sync)
                    </h4>
                    <p className="text-slate-300 text-[11px] mt-0.5 leading-relaxed max-w-xl">
                      ប្រព័ន្ធនឹងបង្កើត HTML ផ្លូវការសម្រាប់កំណត់ហេតុការប្រជុំគ្រូទាំងអស់ របាយការណ៍ហិរញ្ញវត្ថុ ១២ ខែ និង Snapshot JSON រួចបង្ហោះផ្ទាល់ទៅក្នុង Folder ID: <code className="font-mono text-emerald-300">{defaultFolderId}</code>។
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-white/10 text-xs">
                  <div className="bg-white/10 rounded-xl p-3 backdrop-blur-xs">
                    <span className="text-slate-400 text-[10px] block">កំណត់ត្រាកិច្ចប្រជុំគ្រូ</span>
                  ...
                  <div className="bg-white/10 rounded-xl p-3 backdrop-blur-xs">
                    <span className="text-slate-400 text-[10px] block">របាយការណ៍ថវិកា ១២ ខែ</span>
                    <span className="font-bold text-white text-sm">{budgetTransactions.length} ប្រតិបត្តិការ</span>
                    <p className="text-[10px] text-emerald-300 mt-0.5">PB & SIG Budget Summaries</p>
                  </div>

                  <div className="bg-white/10 rounded-xl p-3 backdrop-blur-xs">
                    <span className="text-slate-400 text-[10px] block">ពេលវេលា Auto-Sync ចុងក្រោយ</span>
                    <span className="font-bold text-white text-sm font-times">
                      {driveAutoSyncConfig.lastAutoSyncTime
                        ? new Date(driveAutoSyncConfig.lastAutoSyncTime).toLocaleString('km-KH')
                        : 'ទើបតែដំឡើង'}
                    </span>
                    <p className="text-[10px] text-purple-300 mt-0.5">ស្ថានភាព៖ ដំណើរការល្អ</p>
                  </div>
                </div>
              </div>

              {/* Individual Category Quick Actions */}
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4.5 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h5 className="font-bold text-slate-800 text-sm">កំណត់ត្រាការប្រជុំគ្រូ (Meeting Minutes)</h5>
                        <p className="text-slate-500 text-[11px]">បម្លែងជា HTML ឯកសាររដ្ឋបាលផ្លូវការ</p>
                      </div>
                    </div>
                  </div>
                  <p className="text-slate-600 text-[11px] leading-relaxed">
                    បង្ហោះកំណត់ត្រាកិច្ចប្រជុំ សេចក្ដីសម្រេចចិត្ត និងបញ្ជីវត្តមានរបស់លោកគ្រូ-អ្នកគ្រូជាឯកសារ HTML ទៅ Google Drive។
                  </p>
                  <button
                    onClick={syncAllMeetingsToDrive}
                    disabled={isDriveSyncing}
                    className="w-full flex items-center justify-center gap-2 py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl transition-colors cursor-pointer disabled:opacity-50"
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${isDriveSyncing ? 'animate-spin' : ''}`} />
                    <span>Sync កំណត់ហេតុទាំងអស់ ({teacherMeetings.length})</span>
                  </button>
                </div>
              ...
              <div className="p-4.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                ...
                <div className="space-y-2.5">
                  <label className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50">
                    <div className="flex items-center gap-2.5">
                      <FileText className="w-4 h-4 text-purple-600" />
                      <span className="font-bold text-slate-800">កំណត់ត្រាការប្រជុំគ្រូ (Meeting Minutes HTML)</span>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50">
                    <div className="flex items-center gap-2.5">
                      <Coins className="w-4 h-4 text-emerald-600" />
                      <span className="font-bold text-slate-800">របាយការណ៍ហិរញ្ញវត្ថុ ១២ ខែ (Financial Reports HTML)</span>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="font-bold text-slate-800">មូលដ្ឋានទិន្នន័យទាំងមូល (Full JSON Snapshot)</span>
                    </div>
                    <input
                      type="checkbox"
