...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [isSchoolInfoExpanded, setIsSchoolInfoExpanded] = useState(false);
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<'all' | 'director' | 'teacher' | 'student'>('all');
  ...
  // Filter categories by selected category tab and search query
    ...
    .map(cat => {
      ...
      return {
        ...
        items: cat.items.filter(item =>
          item.labelKh.toLowerCase().includes(q) ||
  ...
  const content = (
    <div className="flex flex-col h-full bg-slate-900 text-slate-200 select-none font-battambang">
      {/* Top Dignified School Header */}
      <div className="p-3.5 sm:p-4 border-b border-slate-800/80 bg-slate-950/70 relative shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-sky-500 p-0.5 shadow-md shrink-0 flex items-center justify-center text-white">
              <School className="w-5 h-5" />
            </div>
            {!isCollapsed && (
              <div className="min-w-0 flex-1">
                <h1 className="text-xs sm:text-sm font-bold font-moul text-amber-300 leading-snug truncate">
                  {language === 'en' ? (schoolProfile.nameLatin || schoolProfile.nameKhmer) : schoolProfile.nameKhmer}
                </h1>
                <p className="text-[10px] sm:text-[11px] text-slate-400 font-medium truncate font-times mt-0.5">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="lg:hidden p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800/80 active:bg-slate-700 transition-colors cursor-pointer min-w-[40px] min-h-[40px] flex items-center justify-center"
        ...
        {/* Academic year badge */}
        {!isCollapsed && (
          <div className="mt-2.5 flex items-center justify-between text-[10px] sm:text-[11px] bg-slate-900/90 rounded-lg px-2.5 py-1 text-slate-300 border border-slate-800">
            ...
            <span className="text-slate-400 font-times shrink-0 ml-1">
      ...
      {/* Main Scrollable Navigation Area (Takes full available height) */}
      <div className="flex-1 overflow-y-auto px-2.5 py-2.5 space-y-3 custom-scrollbar overscroll-contain">
        {/* Compact & Expandable School Info Accordion (Inside scroll area so it never crowds menu items) */}
        {!isCollapsed && (
          <div className="rounded-xl bg-slate-800/40 border border-slate-700/50 overflow-hidden text-xs transition-all">
            <button
              type="button"
              onClick={() => setIsSchoolInfoExpanded(!isSchoolInfoExpanded)}
              className="w-full px-3 py-2 flex items-center justify-between text-left text-slate-300 hover:text-white hover:bg-slate-800/60 transition-colors cursor-pointer"
            >
              <div className="flex items-center gap-2 min-w-0">
                <MapPin className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                <span className="text-[11px] font-medium text-slate-300 truncate">
                  {schoolProfile.commune || schoolProfile.village} • នាយក: {schoolProfile.principalName}
                </span>
              </div>
              <div className="flex items-center gap-1 shrink-0 ml-1 text-slate-400 text-[10px]">
                <span>{isSchoolInfoExpanded ? 'បង្រួម' : 'ព័ត៌មាន'}</span>
                {isSchoolInfoExpanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
              </div>
            </button>

            {isSchoolInfoExpanded && (
              <div className="p-3 border-t border-slate-700/50 space-y-2.5 animate-fade-in bg-slate-900/40">
                <div className="text-[11px] text-slate-300 space-y-0.5">
                  <p className="text-slate-400 text-[10px]">ទីតាំងសាលា៖</p>
                  <p className="font-medium">{schoolProfile.village}, {schoolProfile.commune}, {schoolProfile.district}, {schoolProfile.province}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-1.5 pt-1">
                  {schoolProfile.mapUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center gap-1 px-2 py-1.5 bg-blue-600/30 hover:bg-blue-600/50 border border-blue-500/40 rounded-lg text-[10px] text-blue-300 hover:text-white transition-colors"
                    >
                  ...
                  {schoolProfile.facebookPage && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center gap-1 px-2 py-1.5 bg-sky-600/30 hover:bg-sky-600/50 border border-sky-500/40 rounded-lg text-[10px] text-sky-300 hover:text-white transition-colors"
                    >
        ...
        {/* Quick Search & Filter Controls for Orderly Mobile Navigation */}
        {!isCollapsed && (
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="ស្វែងរកម៉ឺនុយ / Search menu..."
                className="w-full bg-slate-950/80 border border-slate-800 rounded-xl pl-8 pr-7 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
              />
            ...
            {filteredCategories.length > 1 && (
              <div className="flex items-center gap-1 p-1 bg-slate-950/70 rounded-xl border border-slate-800 text-[11px] overflow-x-auto no-scrollbar">
                <button
                  type="button"
                  onClick={() => setSelectedCategoryFilter('all')}
                  className={`flex-1 min-w-max py-1 px-2.5 rounded-lg font-bold transition-all text-center cursor-pointer ${
                ...
                {filteredCategories.map(cat => (
                  <button
                    ...
                    className={`min-w-max py-1 px-2.5 rounded-lg font-bold transition-all text-center cursor-pointer ${
                      selectedCategoryFilter === cat.id
                        ? 'bg-blue-600 text-white shadow-xs'
                        : 'text-slate-400 hover:text-white hover:bg-slate-800/60'
                    }`}
                  >
                    {cat.id === 'director' ? 'រដ្ឋបាល' : cat.id === 'teacher' ? 'គ្រូបង្រៀន' : 'សិស្ស'}
                  </button>
                ))}
              </div>
            )}
        ...
        {/* Role & Item Count Indicator */}
        <div className="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center justify-between">
          <span>{!isCollapsed ? (language === 'en' ? `Role: ${getRoleLabel(currentUser?.role)}` : `សិទ្ធិ: ${getRoleLabel(currentUser?.role)}`) : '•••'}</span>
          {!isCollapsed && (
            <span className="text-[10px] font-mono text-slate-500">
              {displayCategories.reduce((acc, c) => acc + c.items.length, 0)} មុខងារ
            </span>
          )}
        </div>
            ...
            <div key={category.id} className="space-y-1">
              ...
              {!isCollapsed ? (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-[11px] font-bold tracking-wide transition-colors cursor-pointer ${
              ...
              {/* Category Items List */}
              {!isCategoryCollapsed && (
                <div className="space-y-1 animate-fade-in pl-0.5">
                  {category.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs transition-all duration-150 relative min-h-[44px] cursor-pointer active:scale-[0.98] ${
                          isActive
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          {!isCollapsed && (
                            <div className="text-left min-w-0 truncate">
                              ...
                              <span className={`text-[10px] block leading-none font-times mt-0.5 ${isActive ? 'text-blue-100' : 'text-slate-400'}`}>
                                {secondaryTitle}
                              </span>
                            </div>
                          )}
        ...
        {!isCollapsed && displayCategories.length === 0 && (
          <div className="text-center py-6 text-slate-500 text-xs">
            <p>រកមិនឃើញមុខងារ &quot;{searchQuery}&quot; ទេ</p>
            <button
      ...
      {/* Sleek Compact Footer Section: Google Workspace Auth, Theme Toggle, Profile */}
      <div className="p-3 border-t border-slate-800 bg-slate-950/80 space-y-2 shrink-0">
        {/* Google Workspace Status (Compact for director/super_admin/secretary) */}
        {(currentUser?.role === 'director' || currentUser?.role === 'super_admin' || currentUser?.role === 'secretary') && (
          !isCollapsed ? (
            <div className="p-2 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 min-w-0">
                <div className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${googleUser ? 'bg-emerald-600/20 text-emerald-400' : 'bg-slate-800 text-slate-400'}`}>
                  <HardDrive className="w-3.5 h-3.5" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-semibold text-slate-200 truncate">
                    {googleUser ? (googleUser.displayName || googleUser.email || 'Google Account') : 'Google Workspace'}
                  </p>
                </div>
              </div>
              <button
                ...
                className={`p-1 rounded-lg text-xs transition-colors shrink-0 cursor-pointer ${
                  googleUser
                    ? 'text-rose-400 hover:bg-rose-500/20'
                    : 'bg-blue-600 hover:bg-blue-700 text-white font-medium px-2 py-0.5 text-[10px]'
                }`}
            ...
            <button
              onClick={onGoogleAuthClick}
              className={`w-full py-1.5 flex items-center justify-center rounded-xl transition-colors cursor-pointer ${googleUser ? 'text-emerald-400 hover:bg-slate-800' : 'text-slate-400 hover:bg-slate-800'}`}
        ...
        {/* User Profile + Theme Mode + System Settings */}
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} pt-0.5 gap-2`}>
          {!isCollapsed && (
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-amber-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs shadow-xs overflow-hidden shrink-0">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {!isCollapsed && (currentUser?.role === 'director' || currentUser?.role === 'super_admin') && (
              <button
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed lg:static top-0 bottom-0 left-0 z-50 bg-slate-900 border-r border-slate-800 transition-all duration-300 ease-in-out flex flex-col h-full h-[100dvh] ${
          isMobileOpen ? 'translate-x-0 w-[88vw] max-w-[340px] sm:max-w-sm shadow-2xl' : '-translate-x-full lg:translate-x-0'
