...
export const TeacherManagement: React.FC = () => {
  const { teachers, addTeacher, updateTeacher, deleteTeacher, searchQuery, schoolProfile, showToast, currentUser } = useSchool();
  ...
  const [selectedTeacherForCertificate, setSelectedTeacherForCertificate] = useState<Teacher | null>(null);
  ...
  return (
    <div className="space-y-6 font-battambang">
          ...
          >
            ...
            <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={() => setSelectedTeacherForCertificate(teacher)}
                  className="p-1.5 text-amber-700 hover:text-amber-900 hover:bg-amber-50 rounded-lg transition-colors text-xs font-semibold flex items-center gap-1 border border-amber-200/60"
                  title="ចេញវិញ្ញាបនបត្រផ្លូវការ (សាលាបឋមសិក្សា ភ្នំពុំ - នាយកសាលា)"
                >
                  <Award className="w-4 h-4 text-amber-600" />
                  <span className="text-[11px]">វិញ្ញាបនបត្រ</span>
                </button>
              </div>
      ...
      {/* Khmer Ornate Certificate Studio Modal for Teacher */}
      {selectedTeacherForCertificate && (
        <KhmerOrnateCertificateModal
          isOpen={!!selectedTeacherForCertificate}
          onClose={() => setSelectedTeacherForCertificate(null)}
          initialRecipientName={selectedTeacherForCertificate.nameKhmer}
