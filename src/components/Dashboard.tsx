...
import { DashboardSummaryPrintModal } from './DashboardSummaryPrintModal';
import {
  Users,
  GraduationCap,
  School,
...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome & Overview Banner with Angkor Artwork / Custom Image */}
      <div className="rounded-2xl p-5 sm:p-6 text-white shadow-xl relative overflow-hidden border border-indigo-900/60 transition-all duration-300">
        {/* Dynamic Angkor Artwork / Custom Background */}
        <AngkorBannerArtwork
          bannerUrl={schoolProfile.bannerUrl}
          bannerType={schoolProfile.bannerType || 'angkor_twilight'}
          overlayOpacity={schoolProfile.bannerOverlayOpacity !== undefined ? schoolProfile.bannerOverlayOpacity : 80}
        />
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
          <div className="space-y-2 max-w-3xl">
            <div className="flex flex-wrap items-center gap-2.5 text-xs font-semibold">
              ...
              <span className="text-blue-100 font-medium">ឆ្នាំសិក្សា {schoolProfile.academicYear}</span>
              <span className="text-slate-400">•</span>
              <span className="text-amber-300 font-mono">កូដសាលា: {schoolProfile.schoolCode}</span>
              
              {/* Quick Banner Customizer Action */}
              <button
                id="dash-custom-banner-btn"
                type="button"
                onClick={() => setIsBannerCustomizerOpen(true)}
                className="ml-auto lg:ml-2 inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-black/40 hover:bg-black/60 border border-white/20 text-amber-200 hover:text-white text-[11px] font-bold transition-all cursor-pointer backdrop-blur-xs shadow-xs"
                title="ប្តូររូបភាពផ្ទាំងធំ (Change Banner Artwork)"
              >
                <Camera className="w-3 h-3 text-amber-300" />
                <span>ប្តូរផ្ទាំងធំ</span>
              </button>
            </div>

            <div>
              <h2 className="text-2xl sm:text-3xl font-bold font-moul tracking-wide text-white leading-tight drop-shadow-md">
                {schoolProfile.nameKhmer}
              </h2>
              <p className="text-amber-200/95 text-sm font-medium font-times drop-shadow-sm">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              {schoolProfile.mapUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-600/30 hover:bg-red-600/50 border border-red-500/40 rounded-lg text-xs text-red-200 hover:text-white transition-colors backdrop-blur-xs"
                >
              ...
              {schoolProfile.facebookPage && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1 bg-sky-600/30 hover:bg-sky-600/50 border border-sky-500/40 rounded-lg text-xs text-sky-200 hover:text-white transition-colors backdrop-blur-xs"
                >
          ...
          <div className="flex flex-wrap sm:flex-col gap-2 flex-shrink-0">
            <button
              id="dash-print-summary-btn"
              type="button"
              onClick={() => setIsPrintSummaryOpen(true)}
              className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md transition-transform active:scale-95 cursor-pointer backdrop-blur-xs"
              title="បោះពុម្ពរបាយការណ៍សង្ខេបស្ថិតិសាលារៀន (Print Dashboard Summary)"
            >
              <Printer className="w-4 h-4 text-emerald-100" />
              បោះពុម្ពសង្ខេបស្ថិតិ
            </button>
            <button
              id="dash-add-student-btn"
              onClick={() => setActiveTab('students')}
              className="flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-400 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md transition-transform active:scale-95 cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-slate-800/90 hover:bg-slate-700 border border-slate-600/80 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow transition-transform active:scale-95 cursor-pointer backdrop-blur-xs"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-indigo-900/90 hover:bg-indigo-800 border border-indigo-500/40 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow transition-transform active:scale-95 cursor-pointer backdrop-blur-xs"
            >
      ...
      {/* School News & Recent Announcements Card Layout Component */}
      <SchoolNews />
      ...
      {/* Dashboard Summary Print Modal */}
      <DashboardSummaryPrintModal
