...
interface SchoolContextType {
  ...
  // Data Maintenance & Database Health (ការថែទាំទិន្នន័យ និងបណ្ណសារ)
  archiveAcademicYearRecords: (academicYear: string) => { success: boolean; count: number; message: string; exportJson?: string; filename?: string };
  purgeAcademicYearRecords: (academicYear: string) => { success: boolean; count: number; message: string };
  vacuumDatabase: () => { success: boolean; cleanedCount: number; message: string };

  // Session Lifetime & Expiration Warning (ការគ្រប់គ្រងសុពលភាព Session)
  sessionSecondsRemaining: number;
  isSessionWarningOpen: boolean;
  extendSession: () => void;
  triggerSessionWarningDemo: () => void;
...
const DEFAULT_SESSION_DURATION_MS = 15 * 60 * 1000; // 15 minutes
const WARNING_THRESHOLD_SECONDS = 120; // 2 minutes before expiry (120s)
...
export const SchoolProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [sessionExpiresAt, setSessionExpiresAt] = useState<number | null>(() => {
    const saved = localStorage.getItem(`${LOCAL_STORAGE_KEY}_session_exp`);
    if (saved) {
  ...
  const [sessionSecondsRemaining, setSessionSecondsRemaining] = useState<number>(DEFAULT_SESSION_DURATION_MS / 1000);
  ...
  // Language LocalStorage Sync
  useEffect(() => {
    localStorage.setItem(`${LOCAL_STORAGE_KEY}_lang`, language);
  }, [language]);

  // Dark Mode LocalStorage Sync and Class Toggle with Smooth Transitions
  useEffect(() => {
  ...
  const t = (key: string): string => {
    const translations: Record<string, { km: string; en: string }> = {
      dashboard: { km: 'ផ្ទាំងគ្រប់គ្រង', en: 'Dashboard' },
      students: { km: 'គ្រប់គ្រងសិស្ស', en: 'Students' },
      teachers: { km: 'គ្រប់គ្រងគ្រូ', en: 'Teachers' },
      classes: { km: 'ថ្នាក់រៀន', en: 'Classrooms' },
      scores: { km: 'ពិន្ទុ & ចំណាត់ថ្នាក់', en: 'Grades & Ranking' },
      attendance: { km: 'វត្តមាន & សុខភាព', en: 'Attendance & Health' },
      finance: { km: 'ថវិកា & ហិរញ្ញវត្ថុ', en: 'Finance & Budget' },
      reports: { km: 'របាយការណ៍ & QR', en: 'Reports & QR' },
      calendar: { km: 'ប្រតិទិនសិក្សា', en: 'Academic Calendar' },
      accounts: { km: 'គណនី & សិទ្ធិប្រើប្រាស់', en: 'User Accounts' },
      library: { km: 'បណ្ណាល័យ', en: 'Library' },
      school_admin: { km: 'រដ្ឋបាលសាលា', en: 'School Administration' },
      school_management: { km: 'ការគ្រប់គ្រងសាលា', en: 'School Management' }
    };
    if (translations[key]) {
      return language === 'en' ? translations[key].en : translations[key].km;
    }
    return key;
  };
  ...
  const getFormattedGrade = (averageScore: number, gradeLetter?: string): string => {
    if (gradingScaleType === 'letter') {
      ...
      if (averageScore >= 9) return 'A';
      if (averageScore >= 8) return 'B';
      if (averageScore >= 7) return 'C';
      if (averageScore >= 6) return 'D';
      if (averageScore >= 5) return 'E';
      return 'F';
    }
    if (averageScore >= 9) return 'ល្អណាស់';
    if (averageScore >= 8) return 'ល្អ';
    if (averageScore >= 7) return 'ល្អបង្គួរ';
    if (averageScore >= 6) return 'មធ្យម';
    if (averageScore >= 5) return 'ខ្សោយ';
    return 'ខ្សោយណាស់';
  };

  // Session Lifetime Management
  const resetSessionTimer = () => {
    const newExp = Date.now() + DEFAULT_SESSION_DURATION_MS;
    setSessionExpiresAt(newExp);
    localStorage.setItem(`${LOCAL_STORAGE_KEY}_session_exp`, newExp.toString());
    setIsSessionWarningOpen(false);
    setSessionSecondsRemaining(DEFAULT_SESSION_DURATION_MS / 1000);
  };

  const extendSession = () => {
    resetSessionTimer();
    showToast('បានបន្តសុពលភាព Session ១៥ នាទីទៀតដោយជោគជ័យ!', 'success');

    if (currentUser) {
      addActivityLog({
        domain: 'auth',
        actionType: 'update',
        title: `បន្តសុពលភាព Session៖ ${currentUser.nameKhmer}`,
        description: `អ្នកប្រើប្រាស់បានចុចបន្តសុពលភាព Session (+15 នាទី) ដោយសុវត្ថិភាព`,
        entityId: currentUser.id,
        entityName: currentUser.nameKhmer,
        actorName: currentUser.nameKhmer,
        actorRole: getRoleLabel(currentUser.role),
        tags: ['Session', 'បន្តសុពលភាព']
      });
    }
  };

  const triggerSessionWarningDemo = () => {
    const demoExp = Date.now() + 119 * 1000;
    setSessionExpiresAt(demoExp);
    localStorage.setItem(`${LOCAL_STORAGE_KEY}_session_exp`, demoExp.toString());
    setSessionSecondsRemaining(119);
  ...
  // Monitor Session Expiry Every 1 Second
  useEffect(() => {
    if (!currentUser) {
      setIsSessionWarningOpen(false);
      return;
    ...
    const interval = setInterval(() => {
      ...
      setSessionSecondsRemaining(remainingSec);

      if (remainingSec <= WARNING_THRESHOLD_SECONDS && remainingSec > 0) {
        setIsSessionWarningOpen(true);
        ...
        if (currentUser) {
          addActivityLog({
            domain: 'auth',
            actionType: 'logout',
            title: `Session ផុតកំណត់៖ ${currentUser.nameKhmer}`,
            description: `Session បានផុតកំណត់ដោយស្វ័យប្រវត្ត ដោយសារគ្មានសកម្មភាពរយៈពេល ១៥ នាទី`,
            entityId: currentUser.id,
            entityName: currentUser.nameKhmer,
            actorName: currentUser.nameKhmer,
            actorRole: getRoleLabel(currentUser.role),
            tags: ['Session Timeout', 'ផុតកំណត់']
          });
        }
        setCurrentUser(null);
        localStorage.removeItem(`${LOCAL_STORAGE_KEY}_current_user`);
        localStorage.removeItem(`${LOCAL_STORAGE_KEY}_session_exp`);
        showToast('Session របស់អ្នកបានផុតកំណត់ហើយ។ សូមចូលប្រើប្រាស់ម្ដងទៀត។', 'error');
      }
    }, 1000);
  ...
  const login = (identifier: string, password: string) => {
    ...
    if (user) {
      ...
      addActivityLog({
        domain: 'auth',
        actionType: 'login',
        title: `បានចូលប្រព័ន្ធ៖ ${user.nameKhmer}`,
        description: `គណនី ${user.username} (${getRoleLabel(user.role)}) បានចូលប្រើប្រាស់ប្រព័ន្ធជោគជ័យ`,
        entityId: user.id,
        entityCode: user.staffCode || user.studentCode,
        entityName: user.nameKhmer,
        actorName: user.nameKhmer,
        actorRole: getRoleLabel(user.role),
        targetTab: user.role === 'student' ? 'student_portal' : 'dashboard',
        tags: ['ចូលប្រព័ន្ធ', getRoleLabel(user.role)]
      });
  ...
  const loginWithGoogle = async (): Promise<{ success: boolean; message: string; user?: AppUser }> => {
    try {
      ...
      addActivityLog({
        domain: 'auth',
        actionType: 'login',
        title: `ចូលប្រើតាម Google Gmail៖ ${matchedUser.nameKhmer}`,
        description: `គណនី Google (${matchedUser.email}) ឋានៈ ${getRoleLabel(matchedUser.role)} បានផ្ទៀងផ្ទាត់ចូលប្រព័ន្ធ`,
        entityId: matchedUser.id,
        entityName: matchedUser.nameKhmer,
        actorName: matchedUser.nameKhmer,
        actorRole: getRoleLabel(matchedUser.role),
        targetTab: matchedUser.role === 'student' ? 'student_portal' : 'dashboard',
        tags: ['Google OAuth', 'ចូលប្រព័ន្ធ']
      });
  ...
  const logoutApp = () => {
    if (currentUser) {
      addActivityLog({
        domain: 'auth',
        actionType: 'logout',
        title: `បានចាកចេញពីប្រព័ន្ធ៖ ${currentUser.nameKhmer}`,
        description: `គណនី ${currentUser.username} (${getRoleLabel(currentUser.role)}) បានចាកចេញដោយសុវត្ថិភាព`,
        entityId: currentUser.id,
        entityName: currentUser.nameKhmer,
        actorName: currentUser.nameKhmer,
        actorRole: getRoleLabel(currentUser.role),
  ...
  const canAccessTab = (tab: ActiveTab): boolean => {
    ...
    if (role === 'secretary') {
      return ['dashboard', 'homeroom_dashboard', 'ai_teacher', 'activity_history', 'activity_logs', 'school_admin', 'school_management', 'official_documents', 'students', 'transfers', 'household_census', 'teachers', 'classrooms', 'attendance_health', 'calendar', 'reports_qr', 'settings', 'accounts', 'library', 'workspace'].includes(tab);
    }
    ...
    if (role === 'teacher') {
      return ['homeroom_dashboard', 'dashboard', 'ai_teacher', 'activity_history', 'activity_logs', 'school_admin', 'school_management', 'official_documents', 'students', 'transfers', 'household_census', 'classrooms', 'scores', 'attendance_health', 'calendar', 'reports_qr', 'accounts', 'library', 'workspace'].includes(tab);
    }
  ...
  // Data Maintenance & Archive Implementation
  const archiveAcademicYearRecords = (targetYear: string) => {
    if (currentUser?.role !== 'director' && currentUser?.role !== 'secretary') {
      return { success: false, count: 0, message: 'មានតែនាយកសាលា ឬលេខាធិការប៉ុណ្ណោះដែលអាចបង្កើតបណ្ណសារទិន្នន័យបាន!' };
    }

    const yearScores = scores.filter(s => s.academicYear === targetYear);
    const yearAttendance = attendanceRecords.filter(a => a.academicYear === targetYear);
    const yearLessonPlans = lessonPlans.filter(l => l.academicYear === targetYear);
    const yearClassLogs = dailyClassLogs.filter(d => d.academicYear === targetYear);
    const yearTransfers = transfers.filter(t => t.academicYear === targetYear);
    const yearBudget = budgetTransactions.filter(b => b.academicYear === targetYear);

    const totalCount = yearScores.length + yearAttendance.length + yearLessonPlans.length + yearClassLogs.length + yearTransfers.length + yearBudget.length;

    const archivePayload = {
      archiveType: 'AcademicYear_Full_Backup',
      academicYear: targetYear,
      archivedAt: new Date().toISOString(),
      archivedBy: currentUser?.nameKhmer || 'នាយកសាលា',
      schoolCode: schoolProfile.schoolCode,
      schoolName: schoolProfile.nameKhmer,
      recordStats: {
        scoresCount: yearScores.length,
        attendanceCount: yearAttendance.length,
        lessonPlansCount: yearLessonPlans.length,
        classLogsCount: yearClassLogs.length,
        transfersCount: yearTransfers.length,
        budgetCount: yearBudget.length,
        totalRecords: totalCount
      },
      data: {
        scores: yearScores,
        attendanceRecords: yearAttendance,
        lessonPlans: yearLessonPlans,
        dailyClassLogs: yearClassLogs,
        transfers: yearTransfers,
        budgetTransactions: yearBudget
      }
    };

    const exportJson = JSON.stringify(archivePayload, null, 2);
    const filename = `archive_school_year_${targetYear.replace(/\s+/g, '_')}_${Date.now()}.json`;

    // Audit log
    addActivityLog({
      domain: 'system',
      actionType: 'archive',
      title: `បានទាញយកបណ្ណសារទិន្នន័យឆ្នាំសិក្សា ${targetYear}`,
      description: `បានបម្រុងទុកកំណត់ត្រាសរុប ${totalCount} (ពិន្ទុ ${yearScores.length}, វត្តមាន ${yearAttendance.length}, កិច្ចតែងការ ${yearLessonPlans.length}, កំណត់ហេតុថ្នាក់ ${yearClassLogs.length})`,
      entityId: `arch-${Date.now()}`,
      entityCode: targetYear,
      entityName: `បណ្ណសារឆ្នាំសិក្សា ${targetYear}`,
      actorName: currentUser?.nameKhmer || 'នាយកសាលា',
      actorRole: getRoleLabel(currentUser?.role || 'director'),
      targetTab: 'school_management',
      tags: ['បណ្ណសារទិន្នន័យ', targetYear]
    });

    showToast(`បានបង្កើតបណ្ណសារទិន្នន័យឆ្នាំសិក្សា ${targetYear} រួចរាល់ (សរុប ${totalCount} កំណត់ត្រា)`, 'success');
    return { success: true, count: totalCount, message: 'បង្កើតបណ្ណសារជោគជ័យ', exportJson, filename };
  };

  const purgeAcademicYearRecords = (targetYear: string) => {
    if (currentUser?.role !== 'director') {
      return { success: false, count: 0, message: 'មានតែនាយកសាលាប៉ុណ្ណោះដែលអាចសម្អាត/លុបទិន្នន័យចាស់បាន!' };
    }

    const yearScoresCount = scores.filter(s => s.academicYear === targetYear).length;
    const yearAttendanceCount = attendanceRecords.filter(a => a.academicYear === targetYear).length;
    const yearClassLogsCount = dailyClassLogs.filter(d => d.academicYear === targetYear).length;
    const yearLessonPlansCount = lessonPlans.filter(l => l.academicYear === targetYear).length;

    const totalPurged = yearScoresCount + yearAttendanceCount + yearClassLogsCount + yearLessonPlansCount;

    setScores(prev => prev.filter(s => s.academicYear !== targetYear));
    setAttendanceRecords(prev => prev.filter(a => a.academicYear !== targetYear));
    setDailyClassLogs(prev => prev.filter(d => d.academicYear !== targetYear));
    setLessonPlans(prev => prev.filter(l => l.academicYear !== targetYear));

    // Audit log
    addActivityLog({
      domain: 'system',
      actionType: 'purge',
      title: `បានសម្អាតទិន្នន័យចាស់ឆ្នាំសិក្សា ${targetYear}`,
      description: `បានលុបសម្អាតកំណត់ត្រាចាស់ៗចំនួន ${totalPurged} កំណត់ត្រា ដើម្បីបង្កើនល្បឿន និងប្រសិទ្ធភាពប្រព័ន្ធ (Purge Old Records)`,
      entityId: `purge-${Date.now()}`,
      entityCode: targetYear,
      entityName: `សម្អាតឆ្នាំ ${targetYear}`,
      actorName: currentUser?.nameKhmer || 'នាយកសាលា',
      actorRole: 'នាយកសាលា',
      targetTab: 'school_management',
      tags: ['សម្អាតទិន្នន័យ', 'Purge DB', targetYear]
    });

    showToast(`បានសម្អាតកំណត់ត្រាចាស់ៗចំនួន ${totalPurged} នៃឆ្នាំ ${targetYear} រួចរាល់! ប្រព័ន្ធដំណើរការកាន់តែរហ័ស។`, 'success');
    return { success: true, count: totalPurged, message: 'បានសម្អាតជោគជ័យ' };
  };

  const vacuumDatabase = () => {
    const studentIds = new Set(students.map(s => s.id));
    const initialScoresCount = scores.length;
    const validScores = scores.filter(s => !s.studentId || studentIds.has(s.studentId));
    const cleanedScores = initialScoresCount - validScores.length;

    const initialAttendanceCount = attendanceRecords.length;
    const validAttendance = attendanceRecords.filter(a => !!a.date && !!a.grade);
    const cleanedAttendance = initialAttendanceCount - validAttendance.length;

    setScores(validScores);
    setAttendanceRecords(validAttendance);

    const totalCleaned = cleanedScores + cleanedAttendance;

    // Audit log
    addActivityLog({
      domain: 'system',
      actionType: 'sync',
      title: 'បានបង្កើនប្រសិទ្ធភាពទិន្នន័យ (Vacuum & Optimize DB)',
      description: `បានសម្អាតកំណត់ត្រាកំហុស Orphan/Ghost records ចំនួន ${totalCleaned} និងបង្កើនល្បឿន Cache របស់ប្រព័ន្ធ`,
      entityId: `vac-${Date.now()}`,
      entityName: 'Database Optimization',
      actorName: currentUser?.nameKhmer || 'នាយកសាលា',
      actorRole: getRoleLabel(currentUser?.role || 'director'),
      targetTab: 'school_management',
      tags: ['Vacuum DB', 'Optimize Cache']
    });

    showToast(`បានដំណើរការបង្កើនល្បឿនប្រព័ន្ធជោគជ័យ! បានសម្អាត ${totalCleaned} កំណត់ត្រាដែលលើស។`, 'success');
    return { success: true, cleanedCount: totalCleaned, message: 'បង្កើនប្រសិទ្ធភាពជោគជ័យ' };
  };
