...
export const SchoolProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Notifications State - Clean initial state without fake notifications
  ...
  const clearAllNotifications = () => {
    setNotifications([]);
    try {
      localStorage.setItem(`${LOCAL_STORAGE_KEY}_notifications`, JSON.stringify([]));
    } catch (_) {}
    setToastMessage({ text: 'បានលុបការជូនដំណឹងទាំងអស់ជោគជ័យ!', type: 'info' });
  };

  const syncRealSchoolNotifications = () => {
    // Only synchronize genuine pending actions (e.g. pending teacher profile requests)
    // Never auto-generate fake tasks or unsolicited deadlines
    setNotifications(prev => {
      const updated = [...prev];

      // 1. Pending Profile Edit Requests from teachers
      ...
      if (pendingReqs.length > 0) {
        const notifObj: SystemNotification = {
          ...
          timestamp: existingReqIndex >= 0 ? updated[existingReqIndex].timestamp : new Date().toISOString(),
          type: 'alert',
          targetRole: 'director',
